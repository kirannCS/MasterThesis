...
#######################################################################################
#### This script calculates cluster size directly from Mobility Trace (FCD) ###########
#### The micro cloud positions are hard coded #########################################
#### (Upto 2 different micro cloud positions input can be given #######################
#######################################################################################
...
def run():
	...
	for d in xmliter(SumoFloatingDataPath, 'timestep'):
		...
		# Calculates cluster size with precision = 1 sec
		if float(d['@time']) == PrintTime:
			...
			if type(d['vehicle']) == type(_dict):
				...
				Distance1 = DistanceCalc(float(d['vehicle']['@x']), float(d['vehicle']['@y']), 170.0, 174.0)
				...
				if Distance2 <= float(CommRange)/2.0:
					...
					DetectCarInMoreThanOneCluster += 1
                        	...
                        	for each in d['vehicle']:
                                	vid = each['@id']
					Distance1 = DistanceCalc(float(each['@x']), float(each['@y']), 170.0, 174.0)
